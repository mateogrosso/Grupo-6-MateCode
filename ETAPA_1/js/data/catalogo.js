// /js/data/catalogo.js
// Fuente única de productos para TODO el sitio
window.CATALOGO = [
  {
    id:'uspallata',
...
// Helpers globales
window.getDestacados = () => (window.CATALOGO || []).filter(p => p.destacado).slice(0, 4);
window.getProductoById = (id) => (window.CATALOGO || []).find(p => p.id === id) || null;
